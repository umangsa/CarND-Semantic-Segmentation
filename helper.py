...
def augment_image(image, gt_image):
    ...
    if np.random.rand() > 0.5:
        alpha = float(np.random.randint(low = 0, high = 2) )
        image = cv2.multiply(image, np.array([alpha]))

    if np.random.rand() < 0.5:
        beta = np.random.randint(low = 0, high = 5)
...
def save_inference_video(runs_dir, data_dir, sess, image_shape, logits, keep_prob, input_image):
    ...
    if os.path.exists(output_dir):
        shutil.rmtree(output_dir)
    os.makedirs(output_dir)
    output = os.path.join(output_dir, "segmented_output.avi")

    # Run NN on test images and save them to HD
    print('Saving test video to: {}'.format(output))

    # Define the codec and create VideoWriter object
    fourcc = cv2.VideoWriter_fourcc(*'XVID') # Be sure to use lower case
    out = cv2.VideoWriter(output, fourcc, 30.0, image_shape)
